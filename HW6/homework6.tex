...
\begin{itemize}
    ...
    More broadly, I expect making $ct$ smaller will increase the number of iterations until convergence. However, in our case our criteria for determining that the algorithm has converged is when $ct$ is greater than the largest difference between the current value and previous value amongst all possible states; on our fifth iteration, our values are not changed and the greatest largest between current and previous values amongst all states is $0$. Thus, for any $ct > 0$, our number of iterations will be 5. 
